...
export default class NoteWeb extends Component {
   ...
   */
    ...
    // ];
    let nodes = [
      { key: "node0", id: "0", label: "Sample Title 1", content: "This is sample note 1", created:new Date().getTime()},
      { key: "node1", id: "1", label: "Sample Title 2", content: "This is sample note 2", created:new Date().getTime()},
      { key: "node2", id: "2", label: "Sample Title 3", content: "This is sample note 3", created:new Date().getTime()}
    ]
    
    let edges = [
       { key: "edge0", id: "0", from:"0", to:"1", created:new Date().getTime()},
       { key: "edge0", id: "1", from:"0", to:"2", created:new Date().getTime()}
    ]
    // let nodes = [
    ...
    this.fetchNotesAndEdges = this.fetchNotesAndEdges.bind(this);
  ...
  fetchNotesAndEdges(){
    let fetch_nodes = fetch('http://localhost:5000/note/get_all',{
      ...
      .then((myJson) => {
        this.setNodes(myJson['nodes'],myJson['edges']);
      });
  }
    ...
    return (
      <div className="note-web">
        ...
        <div className="main-content">
          <LeftBar key="left" nodes={this.state.nodes} getNote={this.getNote} addNote={this.addNote} state={this.state} fetchNotesAndEdges={this.fetchNotesAndEdges}/>
